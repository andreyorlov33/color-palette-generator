...
export default function generate(asset) {
    ...
    for(let i = 0; i < 10; i++){
        let new_svg = string.replace(/cls-2\b/g, `cls-2_${i}`)
        ...
        if(class_3.base_hex != null){
            new_svg = new_svg.replace(/cls-3\b/g, `cls-3_${i}`)
        ...
        if(class_4.base_hex != null){
            new_svg = new_svg.replace(/cls-4\b/g, `cls-4_${i}`)
    ...
    let html = `<div style="display:flex; flex-direction:row; flex-wrap: wrap; width:100% height: 250px">${html_string}</div>`
